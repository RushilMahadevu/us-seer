...
let cachedCountyData: CountyDataMap | null = null;
let countyFetchPromise: Promise<CountyDataMap> | null = null;

export async function fetchCountyData(): Promise<CountyDataMap> {
  if (cachedCountyData) {
    return cachedCountyData;
  }
  if (countyFetchPromise) {
    return countyFetchPromise;
  }

  countyFetchPromise = (async () => {
...
let cachedCitiesData: CityEntry[] | null = null;
let citiesFetchPromise: Promise<CityEntry[]> | null = null;

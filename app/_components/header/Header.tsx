    ...
    <header className="flex items-center justify-between gap-2.5 sm:gap-4 px-3 py-2 sm:px-5 sm:py-2.5 rounded-2xl border border-border bg-card/95 backdrop-blur-md shadow-xs animate-in fade-in-50 slide-in-from-top-2 duration-400">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        ...
        <button
          ...
          className={`cursor-pointer flex items-center gap-1.5 h-8 sm:h-9 px-2 sm:px-2.5 rounded-xl border border-border/80 bg-muted/40 hover:bg-accent text-xs font-semibold shrink-0 transition-all duration-200 active:scale-97 ${isSimpleMode
            ? "text-amber-400 border-amber-400/40 bg-amber-400/10"
            : "text-muted-foreground hover:text-foreground"
            }`}
        >
          <Feather className="h-3.5 w-3.5" />
          <span className="hidden md:inline">{isSimpleMode ? "Unsimplify" : "Simplify"}</span>
        </button>
